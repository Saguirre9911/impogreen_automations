...
# 4. Entrenamiento del modelo XGBoost (con rezagos)
model_lags = xgb.XGBRegressor(n_estimators=100, random_state=42)
...
# 6. Entrenamiento del modelo XGBoost (sin rezagos)
model_no_lags = xgb.XGBRegressor(n_estimators=100, random_state=42)
...
# Supongamos que XGBoost ya ha sido entrenado con el modelo con rezagos: model_lags

# Importancia de las características
importances = model_lags.feature_importances_
features = X_train_lags.columns
...
plt.barh(
    importance_df["Feature"].head(10)[::-1], importance_df["Importance"].head(10)[::-1]
)
plt.xlabel("Importancia")
plt.title("Importancia de las 10 características más importantes en el modelo XGBoost")
plt.show()
